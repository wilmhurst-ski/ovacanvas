...
export function settingsPlugin(): Plugin {
  ...
  const settingsPath = path.resolve(os.homedir(), '.ovacanvas/settings.json');
  const outputDirectory = path.dirname(settingsPath);

  return {
    name: 'ovacanvas:settings',
    ...
    configureServer(server) {
      server.ws.on('ovacanvas:meta', async ({source, data}, client) => {
        ...
        client.send('ovacanvas:meta-ack', {source});
      });
    },
  };
}
