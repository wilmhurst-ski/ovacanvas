import type {PlayerSettings, Project, StageSettings} from '@ovacanvas/core';
import {Player, Stage} from '@ovacanvas/core';
...
class MotionCanvasPlayer extends HTMLElement {
  ...
  private async updateSource(source: string) {
    ...
    this.player?.togglePlayback(false);
    // Replacing the source definitively retires the previous Player. It is
    // never re-attached, so this is terminal disposal, not sleeping:
    // deactivate() released only the shared WebGL context and left the
    // scenes, audio managers, AudioContexts and subscriptions alive.
    this.player?.dispose();
  ...
  private disconnectedCallback() {
    ...
    // Detaching the host element is not a teardown - the same player may be
    // re-attached. Sleeping suspends scheduling while retaining the scene
    // graph, audio context and shared WebGL context.
    this.player?.sleep();
    this.player?.onRender.unsubscribe(this.render);
  }
