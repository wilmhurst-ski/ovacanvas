 ...
 * @preview
 * ```tsx editor
 * import {Img} from '@ovacanvas/2d';
 * import {all, waitFor} from '@ovacanvas/core';
 * import {createRef} from '@ovacanvas/core';
...
export class Img extends Rect {
  ...
  static {
    // Routed through the single module-lifetime multiplexer rather than
    // registering a second `import.meta.hot.on` handler of its own.
    onAssetReload(({urls}) => {
