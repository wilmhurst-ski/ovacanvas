...
export class Node implements Promisable<Node> {
  ...
  /**
   * The children of this node in the order they are drawn.
   *
   * @remarks
   * The last one is drawn on top. Interaction resolves the topmost target by
   * walking this in reverse, so picking and painting cannot disagree about
   * which node is on top - insertion order alone would ignore z-index.
   *
   * @internal Interaction geometry (CAP-01). Not a frozen public API.
   */
  public drawOrderedChildren(): Node[] {
    return this.sortedChildren();
  }

  /**
   * The local-space box this node's own contents occupy.
   *
   * @remarks
   * Excludes children, unlike {@link cacheBBox}, which unions the whole
   * subtree. An interactive container is therefore reachable through its
   * children rather than through the box they inflate.
   *
   * @internal Interaction geometry (CAP-01). Not a frozen public API.
   */
  public localContentBBox(): BBox {
    return this.getCacheBBox();
  }
