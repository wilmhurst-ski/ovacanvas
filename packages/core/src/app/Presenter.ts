...
const NextSlide = Symbol('@ovacanvas/core/app/NextSlide');
const PreviousSlide = Symbol('@ovacanvas/core/app/PreviousSlide');
...
export class Presenter {
  ...
  public async present(settings: PresenterSettings) {
    if (this.disposed) {
      throw new Error('Cannot present with a disposed Presenter.');
    }
  ...
  public abort() {
    ...
    // Aborting must stop scheduling immediately. Relying on the next frame to
    // notice the aborted signal leaves one queued callback holding this
    // Presenter, its Stage and its Scenes alive.
    this.stopScheduling();
    this.state.current = PresenterState.Aborting;
  }

  /**
   * Permanently release everything this Presenter owns.
   *
   * @remarks
   * Terminal and idempotent. Disposal becomes visible before anything is
   * released so that a frame or a resumed `run()` observes a dead Presenter
   * and fails closed. `abort()` only ends the current presentation; this ends
   * the Presenter, including its Scenes, its Stage and its shared WebGL
   * context, none of which `abort()` ever released.
   */
  public dispose() {
