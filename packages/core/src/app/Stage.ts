...
export class Stage {
  ...
  /**
   * Release the canvas buffers backing this stage.
   *
   * @remarks
   * Terminal and idempotent. Collapsing each canvas to 0x0 is what actually
   * frees the backing store; dropping the reference alone leaves the bitmap
   * alive until the next garbage collection, and a detached 4K buffer is
   * several megabytes. The final buffer is also detached from the document,
   * since the host handed it to the DOM.
   */
  public dispose() {
    if (this.disposed) return;
    this.disposed = true;

    this.finalBuffer.remove();
    for (const canvas of [
      this.finalBuffer,
      this.currentBuffer,
      this.previousBuffer,
