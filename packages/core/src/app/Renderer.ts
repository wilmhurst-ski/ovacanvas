...
export class Renderer {
  ...
  public async render(settings: RendererSettings) {
    if (this.disposed) {
      throw new Error('Cannot render with a disposed Renderer.');
    }
  ...
  /**
   * Permanently release everything this Renderer owns.
   *
   * @remarks
   * Terminal and idempotent. `abort()` only ends the current render; it never
   * released the Scenes, the Stage or the shared WebGL context this object
   * constructed. Disposal becomes visible first so that a resumed render
   * observes a dead Renderer instead of dispatching into it.
   */
  public dispose() {
  ...
  public async renderFrame(settings: RendererSettings, time: number) {
    ...
    try {
      ...
      if (import.meta.hot) {
        import.meta.hot.send('ovacanvas:export', {
          frame,
