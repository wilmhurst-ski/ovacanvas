  ...
  implements Scene<ThreadGeneratorFactory<T>>, Threadable
{
  ...
  /**
   * Permanently release everything this scene owns.
   *
   * @remarks
   * Terminal and idempotent, and distinct from {@link reset}: resetting
   * rewinds presentation and leaves the scene usable, whereas disposing ends
   * the thread generator, releases the time and lifecycle event
   * subscriptions, and empties every dispatcher.
   *
   * Returning the runner rather than dropping it is what lets `finally`
   * blocks inside authored generators run.
   */
  public dispose() {
    if (this.disposed) return;
    this.disposed = true;
    this.runner?.return(undefined);
    this.runner = null;
    this.timeEvents.dispose?.();
    this.lifecycleEvents.dispose();
    this.cache.clear();
    this.reloaded.clear();
    this.recalculated.clear();
    this.thread.clear();
    this.renderLifecycle.clear();
