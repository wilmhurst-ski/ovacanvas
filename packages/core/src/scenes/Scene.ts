...
export interface Scene<T = unknown> {
  ...
  /**
   * Permanently release every resource and subscription owned by this scene.
   *
   * @remarks
   * Disposal is terminal and idempotent, and is a different lifecycle event
   * from {@link reset}: resetting rewinds presentation and keeps the scene
   * usable, disposing ends it.
   */
  dispose(): void;
