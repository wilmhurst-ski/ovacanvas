...
export class AudioManager {
  ...
  /**
   * Terminally release the audio element and the Web Audio graph.
   *
   * @remarks
   * The order matters. `removeAttribute('src')` followed by `load()` is the
   * documented way to make a browser tear down the media pipeline and free
   * the decoder; assigning `src = ''` instead leaves a resolved URL pointing
   * at the document and keeps the element alive.
   */
  public dispose() {
    if (this.disposed) return;
    this.disposed = true;
