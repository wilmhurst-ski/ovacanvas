...
import {AssetReloadEvent, onAssetReload} from './assetEvents';
...
export class AudioResourceManager {
  ...
  private readonly handleAssetReload = ({urls}: AssetReloadEvent) => {
    if (this.disposed) return;
    for (const url of urls) {
      this.lookup.get(url)?.reload();
    }
  };
  ...
  public get(source: string): AudioResource {
    if (this.disposed) {
      throw new Error('Cannot acquire audio from a disposed resource manager.');
    }
  ...
  /**
   * Terminally dispose every tracked resource and close the decoding context.
   */
  public dispose() {
    if (this.disposed) return;
    this.disposed = true;
    this.disposeAssetReload();
    for (const resource of this.lookup.values()) resource.dispose();
    this.lookup.clear();
...
export class AudioResource {
  ...
  /**
   * Abort any in-flight fetch or decode and stop publishing data.
   *
   * @remarks
   * Aborting rather than letting the request settle is what prevents
   * unhandled rejections and zombie decode work after a runtime is gone.
   */
  public dispose() {
    if (this.disposed) return;
    this.disposed = true;
